 ...
 * AMS Child Theme.
 *
 * This file adds the Genesis Connect for WooCommerce notice to the AMS Child Theme Theme.
 *
...
add_action( 'admin_print_styles', 'ams_remove_woocommerce_notice' );
/**
 * Removes the default WooCommerce Notice.
 *
 * @since 2.3.0
 */
function ams_remove_woocommerce_notice() {
...
add_action( 'admin_notices', 'ams_woocommerce_theme_notice' );
/**
 ...
 * @since 2.3.0
 */
function ams_woocommerce_theme_notice() {
	...
	if ( get_user_option( 'ams_woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}
...
add_action( 'wp_ajax_ams_dismiss_woocommerce_notice', 'ams_dismiss_woocommerce_notice' );
/**
 * Adds option to dismiss Genesis Connect for Woocommerce plugin install prompt.
 *
 * @since 2.3.0
 */
function ams_dismiss_woocommerce_notice() {

	update_user_option( get_current_user_id(), 'ams_woocommerce_message_dismissed', 1 );

}

add_action( 'admin_enqueue_scripts', 'ams_notice_script' );
/**
 * Enqueues script to clear the Genesis Connect for WooCommerce plugin install prompt on dismissal.
 *
 * @since 2.3.0
 */
function ams_notice_script() {

	wp_enqueue_script( 'ams_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', [ 'jquery' ], '1.0', true );

}

add_action( 'switch_theme', 'ams_reset_woocommerce_notice', 10, 2 );
/**
 * Clears the Genesis Connect for WooCommerce plugin install prompt on theme change.
 *
 * @since 2.3.0
 */
function ams_reset_woocommerce_notice() {

	global $wpdb;

	$args  = [
		'meta_key'   => $wpdb->prefix . 'ams_woocommerce_message_dismissed', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		'meta_value' => 1, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value
	];
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, 'ams_woocommerce_message_dismissed' );
	}

}

add_action( 'deactivated_plugin', 'ams_reset_woocommerce_notice_on_deactivation', 10, 2 );
/**
 ...
 */
function ams_reset_woocommerce_notice_on_deactivation( $plugin, $network_deactivating ) {
